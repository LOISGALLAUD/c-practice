...
int distribution (int somme);

int billet[5] = {5, 10, 20, 50, 100}; // Types de billets références qui servent pour la conversion.

int conversion (int montantBillet, int indiceBillet) {
	int billetsConvertis = 0;
	
	// Tant qu'on peut retirer des billets de valeur "indiceBillet", on décrémente le montant total.
	while (montantBillet-indiceBillet > 0) {
		billetsConvertis ++;
		montantBillet -= indiceBillet;
	}
	
	// Si le compte est bon, il ne faut pas oublier de compter 1 fois (on pourrait modifier le code et utiliser une structure en do-while pour faire cela.)
...
int distribution (int somme) {
	...
	// Pour chaque valeur de billet référence on convertit en décrémentant au fur et à mesure le montant total.
	for (int i = 4; i >= 0; i--) {
